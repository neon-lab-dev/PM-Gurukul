...
const HeaderDashboard = () => {
  ...
  return (
    <div className="bg-white text-3xl flex justify-between items-center p-4 sticky top-0 z-20 font-Inter">
      <ul>
        <li className="text-xl">Welcome back <span className={`font-semibold text-blue-10`}>{userInfo?.full_name}</span></li>
        <li className={`text-sm mt-[2px] ${!hasKycDetails ? "text-red-500" : "text-green-500"}`}>{hasKycDetails ? "KYC Verified" : "Please complete your KYC."}</li>
      </ul>
